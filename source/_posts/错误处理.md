...
`ArraySymbol`比较简单有是否是常量、数组维数、每一维数组长度三个属性。
...
考虑了各种`Symbol`就来看看真正的`SymbolTable`。

**符号表系统设计**

![截屏2022-10-21 22.11.41](https://raw.githubusercontent.com/hjc-owo/hjc-owo.github.io/img/202210212211169.png)
...
public class SymbolTable {
    ...
    ... // 构造方法们
    public Symbol get(String ident) { ... } // 从符号表当中获取
    public boolean contains(String ident) { ... } // 符号表里有
    public boolean containsInCurrent(String ident) { ... } // 当前符号表里有
    public void put(String ident, Symbol symbol) { ... } // 放到符号表
}
```

考虑到还需要对函数的返回值进行判断，设计了`FuncSymbolTable`，继承`SymbolTable`，添加了函数返回值的属性。

其实对某些`node`文件夹下的文件也进行了修改，将在下一个部分进行说明。
...
- f 类错误 - 无返回值的函数存在不匹配的`return`语句
  ...
  对于题目给出的文法，只需要在`stmt`的类型是`Return`时，判断当前是不是在函数当中（表现为`currentSymbolTable instanceof FuncSymbolTable`），如果在函数当中，但是`expNode != null`，那就报错。

- g 类错误 - 有返回值的函数缺少`return`语句

  g 类错误在`BlockNode`当中处理比较方便。当前在函数当中，并且当前`blockNode`有最后一条语句，并且最后一条语句是`return`语句即可，否则报错。
